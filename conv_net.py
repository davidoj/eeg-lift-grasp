...
from  neon.layers import (FCLayer, DataLayer, CostLayer, ConvLayer,
                          PoolingLayer, DropOutLayer)
from  neon.transforms import RectLin, Logistic, CrossEntropy
from   neon.models import MLP

from preprocessing import fir_lowpass_filter, prepare_freq_bands, butter_lowpass_filter
...
class GalData(Dataset):
    ...
    def readfiles(self, dirname, serlist):
        ...
        """
        basepath = os.path.join(os.pardir,  dirname)
        data = labs = inds = None
    ...
    def prep(self, data):
        ...
        #Xs.append(fir_lowpass_filter(data,3,3,500,20))
        
        #Xs.append(prepare_freq_bands(data,[[5,20]]))

        #data = np.concatenate(Xs,axis=1)

        data = butter_lowpass_filter(data,50,500)
        #if self.subj != 8:
    ...
    def load(self, **kwargs):
        ...
        if validate:
            ...
            trainlist = range(1,9)
            testlist = [9,10]
        if self.subj == 2 and 2 in trainlist:
            trainlist.remove(2)
        train, trainlabs, _ = self.readfiles('train', trainlist)
        test, testlabs, self.testinds = self.readfiles(testpath, testlist)
...
class ConvNet(object):
    ...
    def __init__(self, backend, dataset, subj):
        ...
        self.model = MLP(num_epochs=5, batch_size=128, layers=self.layers)
...
def run(subj):
    ...
    for winsize in winlist:
        ds.setwin(winsize=winsize, subsample=8)
